### **大规模集群节点管理工具 “Oasis” 方案设计文档**

#### **1. 整体设计**

##### **1.1. 项目目标**

为 OpenCloudOS 设计并开发一款名为 “Oasis” 的大规模集群节点管理工具。该工具旨在提供一个高性能、高可用、安全可靠的解决方案，用于对 100 至 1000+ 规模的节点集群进行高效的管理，并保证核心通信延迟在 50ms 以内。
...
graph TD
    ...
    subgraph "Oasis 系统"
        ...
        subgraph "应用层"
            Admin["管理员 / CLI"]:::admin
            Server["Oasis 服务端 (Rust)"]:::server
            Agent["Oasis 客户端 (Rust)"]:::agent
        end
...
- **高可用协调：** Server 集群的选主通过 **KV + CAS + TTL** 实现。约定键 `kv:OASIS-LEADER`，值为 leader 实例 ID；
...
- **Streams**：
  ...
  - `OASIS-TASKS`：work-queue 语义，`replicas=3`，`storage=file`，`discard=old`，`max_age=24h`，`duplicates=2m`，配置 `max_deliver` 与 DLQ `tasks.DLQ`；subjects: `tasks.exec.>`
  - `OASIS-RESULTS`：结果/审计较长保留期，便于查询与追溯；subjects: `results.>`
  - `OASIS-AUDIT`：关键操作与安全事件长保留，便于合规；subjects: `audit.>`

- **KV 结构**：

  - `kv:OASIS-NODES/<agentId>/facts`（版本化，非 TTL）
  - `kv:OASIS-NODES/<agentId>/heartbeat`（TTL = 2 x 心跳间隔）
  - `kv:OASIS-NODES/<agentId>/labels`（可由 Server/CLI 变更）

- **对象存储**：`obj:oasis.artifacts`，文件元数据包含 `sha256`、`size`、`content-type`，Agent 下载后校验。
...
subjects: audit.>]
  ...
  subgraph KV
    K1[kv:OASIS-NODES/<agentId>/facts]
    K2[kv:OASIS-NODES/<agentId>/heartbeat]
    K3[kv:OASIS-NODES/<agentId>/labels]
    KL[kv:OASIS-LEADER]
  end
  subgraph ObjectStore
    O[obj:OASIS-ARTIFACTS]
  end
...
##### **3.1. Oasis Server (服务端)**

作为系统的业务逻辑编排中心，其核心职责是响应管理请求，并与 JetStream 交互。
...
- **交付节奏**
  ...
  - 每日：可运行、CI 通过、小步提交（含简要变更说明）。
  - 每周：版本标签与变更记录（CHANGELOG），可重现演示步骤。
