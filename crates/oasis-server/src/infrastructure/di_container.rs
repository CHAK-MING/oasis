...
impl InfrastructureDiContainer {
    ...
    fn create_node_repository(&self) -> Arc<dyn NodeRepository> {
        Arc::new(
            crate::infrastructure::persistence::NatsNodeRepository::new(
    ...
    fn create_task_repository(&self) -> Arc<dyn TaskRepository> {
        Arc::new(
            crate::infrastructure::persistence::NatsTaskRepository::new(
                self.jetstream.clone(),
            )
    ...
    fn create_rollout_repository(&self) -> Arc<dyn RolloutRepository> {
        Arc::new(
            crate::infrastructure::persistence::NatsRolloutRepository::new(
                self.jetstream.clone(),
            )
    ...
    fn create_file_repository(&self) -> Arc<dyn FileRepository> {
        Arc::new(
            crate::infrastructure::persistence::NatsFileRepository::new(
                self.jetstream.clone(),
            )
    ...
    fn create_selector_engine(&self) -> Arc<dyn SelectorEngine> {
        Arc::new(
            crate::domain::services::selector_engine::CelSelectorEngine::new()
