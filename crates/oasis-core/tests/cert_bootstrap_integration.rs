    ...
    tokio::spawn(async move {
        ...
        while let Some(msg) = subscriber.next().await {
            ...
            if let Some(reply_subject) = msg.reply {
                ...
                let _ = nats_client
                    .publish(reply_subject, response_bytes.into())
                    .await;
            }
        }
    })
...
async fn test_e2e_agent_bootstrap_with_real_nats() {
    ...
    let ca_listener = tokio::spawn(async move {
        ...
        while let Some(msg) = subscriber.next().await {
            ...
            println!(
                "📨 Received CSR request from agent: {}",
...
async fn test_bootstrap_with_invalid_token_fails() {
    ...
    let ca_listener = tokio::spawn(async move {
        ...
        let mut subscriber = nats_client_clone.subscribe("oasis.ca.csr").await.unwrap();
...
async fn test_expired_bootstrap_token_falls_back_to_enrollment_secret_and_connects() {
    ...
    tokio::fs::copy(&workspace_ca_cert, &ca_cert_path)
        .await
        .unwrap();
    tokio::fs::copy(&workspace_ca_key, &ca_key_path)
        .await
        .unwrap();
    ...
    assert!(
        bootstrap_result.is_ok(),
        "bootstrap fallback should succeed"
    );
...
async fn test_certificate_renewal_can_reconnect_managed_nats_client() {
    ...
    let renewed_cert = tokio::fs::read_to_string(agent_certs_dir.path().join("nats-client.pem"))
        ...
        .unwrap();
    assert_ne!(
        initial_cert, renewed_cert,
        "renewal should replace certificate contents"
    );
    ...
    assert_eq!(
        *generation_rx.borrow(),
        1,
        "managed client should publish a new generation"
    );
